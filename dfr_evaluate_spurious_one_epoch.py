...
def initialize_model(model_name, setup_key):
    ...
    if model_name == 'mlp-1':
        ...
        model = ResNet18(dataset=dataset_name).cuda()
    elif model_name == 'advprop-resnet18':
...
def initialize_checkpoint(model_name, setup_key, ckpt_path):
    model, _ = initialize_model(model_name, setup_key=setup_key)
...
def dfr_on_validation_eval(c, all_embeddings, all_y, num_retrains=20, num_classes=10, preprocess=True, best_coeff=None, best_intercept=None):
    coefs, intercepts = [], []
    if preprocess:
    ...
    logreg.fit(x_val[:num_classes], np.arange(num_classes))
...
def main():
    ...
    if args.random_init_ckpt:
        ...
        dfr_results = {}
        model, num_classes = initialize_model(args.model_name, setup_key=args.setup_key)
        base_model_results = {}
